...
Version 0.1.0 is the first release. Milestones 1 to 12 of [PLAN.md](PLAN.md) are done: the crate routes single traces and differential pairs in all three modes (highlight collisions, walk around, shove) with via placement, layer switching, undo, the optimizer and the rounded corner modes, drags traces, vias, bundles and footprints, tunes length and skew with meanders, carries arcs, exposes a host facing session API (`router::Router`) fed by a plain data board snapshot and answering with preview frames and commit diffs, and records sessions for replay. Ten of the eleven cases of KiCad's own regression corpus replay to KiCad's counts. The LibrePCB integration is built and manually tested on a real project, but it lives on a fork branch and is not upstream, see [doc/librepcb-integration.md](doc/librepcb-integration.md). Every ported routine cites its KiCad origin; tolerance sensitive routines were diffed against compiled transcriptions of the C++.
