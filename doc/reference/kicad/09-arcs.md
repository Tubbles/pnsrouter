...
Correction from slice 8b (2026-09-12): of the five additions above only `PnsNewGeometryKind::Arc` and `PnsNewItem::mid` were right for this host. `PnsShapeKind::Arc`, `PnsShape::mid` and `PnsArcGeometry` have no producer, because the snapshot side flattens LibrePCB's curved copper before it crosses (`boardpnssnapshot.cpp:106`) and has no arc adder, and the preview needs no flattening step at all: the chain already carries its arcs approximated at 1000 nm and `PnsPreviewItem` copies those points, so flattening at 5000 nm would make the preview coarser. `doc/librepcb-integration.md` section 4.9 has the host's account.
