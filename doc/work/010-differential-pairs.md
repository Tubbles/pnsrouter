...
Status: in progress (started 2026-09-10 with the reference note). The value layer is in: the coupling geometry, `DiffPair`, the gateway builders and the fit. So is the optimizer's pair path and `topology::simplify_line`. The placer and the facade are not.
