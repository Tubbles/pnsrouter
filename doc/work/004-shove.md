...
- [x] Phase 3: solids, cluster assembly, walkaround escalation (`TryWalk`).
- [x] Phase 4: vias, via hulls, lone via shove, via fanout drag, reverse via collision.
- [x] Shove mode in the placer (`rh_shove_only`) with the node handoff documented in doc/reference/kicad/03 section 9.3.
- [x] Scenario tests: push a parallel trace aside, retreat and observe springback, shove a via, budget exhaustion returns incomplete.
