...
- **Differential pair optimizer path** (milestone 10, second slice): `optimizer::Optimizer::optimize_diff_pair`, KiCad's `OPTIMIZER::Optimize( DIFF_PAIR* )`, with `merge_dp_segments`, `merge_dp_step`, `coupled_bypass`, `verify_dp_bypass` and `find_coupled_vertices`. It is a second entry point that shares nothing with the single line one: no effort flag reaches it, no constraint runs and the cost estimator is not consulted. What decides is coupled length, and a merge is refused when it would cost more than a tenth of what the pair had. KiCad's loop over the two lanes has no bound and spins forever on a merge that keeps succeeding without shortening a chain; here it is bounded by the same `optimizer::MERGE_PASS_LIMIT` as the single line path. The lane to lane check inside it is an ordinary clearance test through the rule resolver, not the pair gap, so the optimizer can bring the two lanes closer than the gap the route was fitted at; that is KiCad's behaviour and only the coupled length score pushes back.
- `topology::simplify_line`, KiCad's `TOPOLOGY::SimplifyLine`, which drops the collinear vertices of a track already in the node by removing it and adding the simplified shape back. Every segment of the track is a new item afterwards, so a caller holding an `item::ItemId` into it has a stale handle. The pair placer's `FixRoute` is its only caller in KiCad.
