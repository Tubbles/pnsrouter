...
- **Dragging** (milestone 9): `router::Router::start_dragging` starts a segment, corner or via drag on one object, `move_to` and `fix_route` drive it, and `eventlog::SessionEvent::StartDragging` records it. All three routing modes drag a segment or a corner: highlight collisions, walk around, and shove, the last driving the shove engine with the dragged line as its head. The via drag is not there yet and moves nothing. `rules::RuleResolver` is unchanged.
...
- Nine of the eleven KiCad regression cases now reach KiCad's own counts of added and removed items, up from four, and all eleven leave nothing colliding. The two that miss diverge in the board snapshot the test harness builds, which `tests/kicad_replay.rs` documents.
