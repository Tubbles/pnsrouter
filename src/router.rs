...
///
/// A single track placer emits segments, arcs and vias and nothing else
/// (`pcbnew/router/pns_line_placer.cpp:1675`, `:1690`, `:1704`).
#[derive(Copy, Clone, PartialEq, Debug)]
pub enum NewGeometry {
  ...
  /// A curved track, in KiCad's three point form.
  ///
  /// The same `{ start, mid, end, width }` a
  /// [`crate::snapshot::WorldGeometry::Arc`] carries, and the same form
  /// KiCad's own router log writes (`pcbnew/router/pns_logger.cpp:245`).
  /// There is no centre and no angle: those are computed on demand and
  /// neither survives a round trip exactly, see note 09 section 7.4.
  ///
  /// A host whose file format cannot store an arc has to refuse the two
  /// rounded corner modes at its settings boundary rather than flatten
  /// one here, because a flattened arc is a different track and the next
  /// session would read it back as a polyline.
  Arc {
    /// The first endpoint.
    start: Vec2,
    /// A point on the curve between the two endpoints.
    mid: Vec2,
    /// The second endpoint.
    end: Vec2,
    /// The full width in nanometres.
...
impl Router {
  ...
  ///
  /// `Kind::ARC` is in the mask because KiCad filters the three lists on
  /// `IsVirtual()` and on nothing else
  /// (`pcbnew/router/pns_router.cpp:891`, `:897`): an arc reaches both
  /// sides of a commit, the added one when a rounded corner mode places
  /// it and the removed one when a shove pushes an existing arc track
  /// aside.
  ...
  ///
  /// [`None`] for a body no [`NewGeometry`] describes, which is a solid,
  /// a hole, or a handle the arena no longer has.
  /// [`Router::is_committable`] has already dropped all three before this
  /// is reached, so the arm is the one that says so rather than one a
  /// caller can observe; KiCad's own `createBoardItem` has the same
  /// unreachable `default:` (`pcbnew/router/pns_kicad_iface.cpp:2858`).
