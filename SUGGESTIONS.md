...
- **The LibrePCB fork will not compile against the next submodule bump until one enum value is dropped.** `StartError::MultiDragUnsupported` is gone, because more than one segment is now a multi drag rather than a refusal, and `StartError::ComponentDragUnsupported` went with it when component drag landed later the same day, because a set of nothing but pads is now a component drag. Every shape of a non empty item set has an algorithm, so neither value has a successor. The fork's `pns-router` branch names the old value in `libs/librepcb/rust-core/src/ffi/router_ffi.rs`, `libs/librepcb/rust-core/ffi.h`, `libs/librepcb/core/project/board/boardpnsrouter.{h,cpp}` and `libs/librepcb/editor/project/board/fsm/boardeditorstate_routetrace.cpp`. Deleting the variant and its arms is enough; the host passes at most one id, so it could never see either value.
...
- **`ffi_pnsrouter_is_dragging` will answer false during a component drag** once the LibrePCB host can start one. It is `RouterState::DragSegment` on the fork's branch and there are four states now, not three, so it has to become "either drag state". Nothing on the host reaches a component drag today, because the drag gesture hands `start_dragging` at most one host id and that id is never a pad, so this is a latent bug rather than a live one.
