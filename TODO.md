...
- **Fixed 2026-09-24: a pair route with intermediate clicks commits duplicate copper.** Two KiCad defects, both fixed as deviations: the angled continuation gateways step by the sine instead of the tangent and never fit (note 07 erratum E19), so the leg after a fix could not turn by 45 degrees at once; and erratum E12's sticky success survived a fix, so the failed move answered with the fixed leg and the next fix wrote it again. Details, and what the fit still cannot do (a leg makes at most two corners, so the reported (9, 5) click leaves the pair heading north and the target unreachable, now refused rather than duplicated), in `doc/log/2026-09-24.md`. Regression tests in `tests/diff_pair_placer.rs` and `tests/dp_meander_placer.rs`.
